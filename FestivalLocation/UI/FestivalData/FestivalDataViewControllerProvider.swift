...
class FestivalDataViewControllerProvider: DefaultViewControllerProvider {
    ...
    override func createViewController(token: Navigation, context: AnyContext<Navigation>) -> ViewController? {
        guard token == .festivalData, let dataRepository, let locationMonitor else {
            return nil
        }
        let viewModel = FestivalDataViewModel(dataRepository: dataRepository, locationMonitor: locationMonitor)
